...
| Method / ancestor / visibility matrix | Initial regression now covers include+prepend ancestor order, `super` chaining, protected/private `respond_to?`, `public_send`, and private `respond_to_missing?` | Extend the matrix with singleton class/module-function cases, `extend` ordering, and `Method#super_method` across prepended modules |
