...
| Encoding and binary string boundary | Binary file reads now tag returned strings as `ASCII-8BIT`; `String#encoding` returns `Encoding` objects, and regressions cover `String#b`, `force_encoding`, `byteslice`, and invalid-byte file reads | Next layer is real byte-length storage for embedded NULs plus transcoding/invalid-encoding behavior beyond UTF-8 vs ASCII-8BIT tags |
...
- 2026-05-28: Encoding binary boundary — strings now carry a lightweight UTF-8 vs ASCII-8BIT tag; binary `File`/`IO` reads, `gets`, and `getc` preserve ASCII-8BIT; `String#encoding` returns the runtime `Encoding` object; `String#b`, `String#force_encoding`, `String#byteslice`, `dup`, and `clone` preserve or update the tag; Ruby 4.0.4-shaped regression added for invalid binary bytes. Embedded NUL byte lengths and real transcoding remain open; 604 regressions green
