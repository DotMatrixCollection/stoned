...
- 2026-05-17–18: Stage 4/6 compatibility sweep (round 1) — `Proc#curry` (real partial application via Ruby prelude closure); `Integer#[]` (bit indexing, LSB=0); `Hash.[]` (create from flat key/value pairs or array-of-pairs); string bang methods (`upcase!`, `downcase!`, `strip!`, `lstrip!`, `rstrip!`, `chomp!`, `chop!`, `reverse!`, `capitalize!`, `swapcase!`, `gsub!`, `sub!` — delegate to non-bang, return nil on no-change, binding updated via existing mutation propagation); `Integer()` with radix prefixes (0x hex, 0b binary, 0o octal, leading-zero octal); `Integer()` and `Float()` now raise `ArgumentError` on invalid strings (strict end-pointer validation); frozen Array `<<`/`push`/`append`/`concat` and frozen Hash `[]=` now raise FrozenError; `at_exit` properly registers blocks and runs them LIFO at program exit via `AtExitHandler` linked list in Eval struct; custom exception `initialize` with default messages called by both `raise ClassName` and `raise ClassName, msg` paths; `Exception#initialize(msg)` prelude entry sets `@message` for `super(msg)` chains; `Object#equal?` uses pointer identity for strings/arrays/hashes/classes; `puts` no longer double-newlines strings already ending in `\n`; `String#each_char` blockless returns array; modulo operator matches Ruby sign-of-divisor semantics for both int and float; `MatchData#named_captures`, `#names`, `#[]` with Range; `srand` implemented; `Array#zip` with block returns nil; `Class#methods` now includes user-defined class methods (keyed as `"self.name"` in class_env); `Array#transpose`, `Array#assoc`/`rassoc`; `gsub`/`sub` with Hash replacement (both regex and string patterns); 457 regressions green
