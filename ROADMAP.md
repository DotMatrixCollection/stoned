...
- 2026-05-17: Stage 4/6 compatibility sweep — `Proc#curry` (real partial application via Ruby prelude closure); `Integer#[]` (bit indexing, LSB=0); `Hash.[]` (create from flat key/value pairs or array-of-pairs); string bang methods (`upcase!`, `downcase!`, `strip!`, `lstrip!`, `rstrip!`, `chomp!`, `chop!`, `reverse!`, `capitalize!`, `swapcase!`, `gsub!`, `sub!` — delegate to non-bang, return nil on no-change, binding updated via existing mutation propagation); `Integer()` with radix prefixes (0x hex, 0b binary, 0o octal, leading-zero octal); frozen Array `<<`/`push`/`append`/`concat` and frozen Hash `[]=` now raise FrozenError; `at_exit` properly registers blocks and runs them LIFO at program exit via `AtExitHandler` linked list in Eval struct; custom exception `initialize` with default messages now called by `raise ClassName` and `raise ClassName, msg` paths, both route through `ClassName.new`; `Exception#initialize(msg)` prelude entry sets `@message` for `super(msg)` chains; `Gem::MissingSpecVersionError` shim gets its own `initialize` to avoid double-wrapping; 448 regressions green
