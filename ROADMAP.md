...
|---|---|---|
| Tiny pure-Ruby gem lifecycle | `colorize` 1.1.0 installs end-to-end; `require "colorize"` + `require "colorized_string"` work; `"hi".red/.blue.bold/.green`, `colorize(:color=>:magenta,:background=>:cyan)`, `uncolorize`, `ColorizedString.new("hi").red/.blue.bold`, `ColorizedString["world"].green`, `ColorizedString#length`/`upcase`/`to_s`, and a `String.color_samples`-style `permutation` + `ColorizedString.new(str)` path all produce correct output; multi-digit octal escapes (`\033`), CVAR semantics, `String.new` dispatch, and `String` subclass built-in method proxying all fixed | Next: add more zero-dep gems to the probe fixture |
| Method / ancestor / visibility matrix | Regression now covers include+prepend ancestor order, `super` chaining, protected/private `respond_to?`, `public_send`, private `respond_to_missing?`, `Method#super_method`/`Method#call` across prepended modules, Ruby 4.0.4 class-side `extend` ordering, singleton-class inheritance/visibility, and `module_function` instance/module visibility | Keep extending from real Ruby 4.0.4 mismatches, especially singleton objects and module hooks |
