...
- Enumerable: `filter_map`, `grep`, `grep_v`, `find_all`/`filter` aliases, `each_slice(n)`, `each_cons(n)`, `take_while`, `drop_while`; `each_slice` and `each_cons` return an array when called without a block
