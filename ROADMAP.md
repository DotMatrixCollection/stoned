...
|---|---|---|
| Tiny pure-Ruby gem lifecycle | `colorize` 1.1.0 installs end-to-end (`gem install` network path), `require "colorize"` works, `"hi".red`/`.blue.bold`/`.colorize(:green)` produce correct ANSI escape sequences; CVAR semantics via extended modules fixed; `String.new` dispatch fixed; multi-digit octal escapes fixed | Next: `ColorizedString` class API, `String#disable_colorization` state isolation between test runs; add more zero-dep gems to the probe fixture |
