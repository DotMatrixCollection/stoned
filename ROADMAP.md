...
|---|---|---|
| Tiny pure-Ruby gem lifecycle | `colorize` 1.1.0 installs end-to-end; `require "colorize"` + `require "colorized_string"` work; `"hi".red/.blue.bold/.green`, `colorize(:color=>:magenta,:background=>:cyan)`, `uncolorize`, `ColorizedString.new("hi").red/.blue.bold`, `ColorizedString["world"].green`, `ColorizedString#length`/`upcase`/`to_s`, and a `String.color_samples`-style `permutation` + `ColorizedString.new(str)` path all produce correct output; multi-digit octal escapes (`\033`), CVAR semantics, `String.new` dispatch, and `String` subclass built-in method proxying all fixed; `inflector_probe` exercises dry-inflector-style patterns: Regexp-backed rule arrays, `gsub` with string-backreference replacements (`\1`), block form with `$1`, `camelize`/`underscore`/`dasherize`/`humanize` via chained `gsub`; `gsub` empty-match position bug fixed (anchored `s?$` was re-scanning from stale `pos` after empty match, causing exponential output) | Add more zero-dep gems — next interesting targets: a formatter/template gem (exercises `String#%`, sprintf), or a data-structure library using Comparable + Enumerable |
| Method / ancestor / visibility matrix | Regression now covers include+prepend ancestor order, `super` chaining, protected/private `respond_to?`, `public_send`, private `respond_to_missing?`, `Method#super_method`/`Method#call` across prepended modules, Ruby 4.0.4 class-side `extend` ordering, singleton-class inheritance/visibility, `module_function` instance/module visibility, `inherited`/`included`/`prepended`/`extended` lifecycle hooks, `super` in class methods forwarding args correctly, and singleton methods on individual objects | Keep extending: `remove_method`/`method_removed` hook, `undef_method`, singleton class include? reflecting extend, Module#ancestors including singleton-extended modules |
| Encoding and binary string boundary | Strings carry a true `byte_len` field; `\x00`/`\0` escapes in source literals now work; all UTF-8 helpers (`utf8_char_count`, `utf8_char_at`, `utf8_byte_offset_for_char`, `utf8_char_index_for_byte`, `utf8_ascii_only`) now take an explicit `byte_len` parameter — no `strlen` anywhere on the hot string path; `bytesize`, `bytes`, `byteslice`, `+`, `<<`, `empty?`, `ascii_only?`, `length`/`size`, `chars`, `each_char`, `split`, `[]`, `[]=`, `index`, `rindex`, `ljust`/`rjust`/`center`, and `ord` all use `byte_len` and survive embedded NUL bytes | Remaining: no transcoding; no `encode` with external encoding conversion; regex on strings with embedded NULs depends on reginold's NUL tolerance |
...
- keep tightening Rational/Complex/Float edge cases now that real Rational and Complex classes exist
...
- `for var in iterable ... end` loop with correct MRI scope semantics (loop variable leaks into the enclosing scope), including array, range, hash, and `to_a`-backed arbitrary iterables
...
- Enumerable: `find_index`/`index`, `reverse_each`, `cycle`, `partition`, `flat_map`/`collect_concat`, `filter_map`, `grep`, `grep_v`, `find_all`/`filter` aliases, `one?`, `each_slice(n)`, `each_cons(n)`, `chunk`, `chunk_while`, `slice_when`, `slice_before`, `take_while`, `drop_while`; `each_slice` and `each_cons` return an array when called without a block
...
- `String#clear` stub; `String#getbyte`/`setbyte`; `String#[]` with Regexp, String, and capture-group index arguments
...
- 2026-05-20: float rationalization sweep — `RangeError` and `FloatDomainError` are now real builtins (`FloatDomainError < RangeError`), `Float#to_r` now returns the exact binary rational when it fits the current integer model instead of the old fixed `1e7` decimal approximation, `Float#rationalize` now exists with convergent-based simplification (defaulting to a half-ULP neighborhood, so values like `0.1` and `1.0/3.0` rationalize to `1/10` and `1/3`), non-finite float `to_r`/`rationalize` now raise `FloatDomainError`, nil `to_r`/`to_c` conversions return real Rational/Complex values, and Struct gained keyword-init/class-method reflection plus value/pair enumerator helpers; 602 regressions green
...
- 2026-05-28: UTF-8 helper `byte_len` migration — `utf8_char_count`, `utf8_char_at`, `utf8_byte_offset_for_char`, `utf8_char_index_for_byte`, and `utf8_ascii_only` all now accept an explicit `size_t byte_len` parameter; `strlen` loop removed from every function; all call sites in `eval_dispatch_primitives.c` (`chars`, `each_char`, `split`, `[]`, `[]=`, `index`, `rindex`, `ljust`/`rjust`/`center`, `length`/`size`) pass `recv.byte_len`; `[]=` result uses `val_string_n` with correct `byte_len`; 605 regressions green
- 2026-05-28: True byte-length string model — `Value` now carries `byte_len` for all strings; `\x00`/`\0` escapes now legal in string literals (lexer, token, AST, parser, eval pipeline all updated); `bytesize`, `bytes`, `byteslice`, `+`, `<<`, `empty?`, `ascii_only?`, `length`/`size` (for ASCII-8BIT), and binary file reads all use `byte_len` and survive embedded NUL bytes; `val_equal` for strings uses `memcmp`; `val_inspect` escapes high bytes in binary strings; binary fixture `binary_nul.bin` (5 bytes with embedded NUL) added; 605 regressions green
- 2026-05-28: `case/when` local variable scoping, `Array#to_ary`, paint gem probe — sema `collect` now recurses into `NODE_CASE`/`NODE_WHEN` arms so variables assigned inside `when` bodies are visible after the `case` block (was silently treated as method calls); `Array#to_a` and `Array#to_ary` added to `dispatch_array` and both `respond_to?` tables (`builtin_primitive_responds_to` and `primitive_class_responds_to_name`); paint 2.3.0 gem now works: named colors (`:red`), combined effects (`:blue, :bright`), hex strings (`"#ff0000"`), RGB arrays (`[255,128,0]`), mode switching (0/256/truecolor), `detect_mode`, `wrap`, `color`, and `%` template substitution all pass; 607 regressions green
- 2026-05-28: String subclass built-in proxying — user classes that inherit from String (e.g. `ColorizedString < String`) now work: `new` stores initial content in `@__str__` backing ivar; `dispatch_object` proxies all unresolved method calls to `dispatch_string` via the backing; mutating methods (`<<`, `replace`, `clear`) update the backing and return self; `to_s`/`to_str` return the backing string; the generic `#<ClassName:0xPtr>` `to_s` path now exempts String subclasses so they fall through to the proxy; `class_is_a_named_class` check in the early `to_s` guard in `eval_dispatch.c` added; 606 regressions green
- 2026-05-28: Multi-digit octal escape sequences — `\033`, `\077`, `\177` etc. now decode correctly in both plain `"..."` strings and interpolated strings; `\0` still yields NUL; `case '0'` in interpolated scanner merged into the unified `'0'`-`'7'` octal handler; the old `val==0` error guard removed; regression test `string_octal_escapes` added; colorize gem `"hi".red`/`.blue.bold`/`.colorize(:green)` now produce correct ANSI escape sequences end to end; 606 regressions green
- 2026-05-28: Encoding binary boundary — strings now carry a lightweight UTF-8 vs ASCII-8BIT tag; binary `File`/`IO` reads, `gets`, and `getc` preserve ASCII-8BIT; `String#encoding` returns the runtime `Encoding` object; `String#b`, `String#force_encoding`, `String#byteslice`, `dup`, and `clone` preserve or update the tag; Ruby 4.0.4-shaped regression added for invalid binary bytes; 604 regressions green
- 2026-05-30: Module lifecycle hooks + super arg forwarding + gsub empty-match fix — `Class.inherited(subclass)` hook now fires when a class is subclassed (walks superclass chain for the first definition; does NOT fire on reopen); `super` in class methods now forwards the current method's arguments correctly (was looking up bare `method_name` in class_env but class methods are stored as `"self.method_name"`, so args were silently dropped); `gsub` empty-match position bug fixed — after an empty-match at `m.beg`, the code was emitting `s[pos]` (stale old position) and advancing `pos++` instead of emitting `s[m.beg]` and advancing to `m.beg+1`, causing exponential output for anchored optional-match patterns like `/s?$/`; `inflector_probe` added as zero-dep gem probe (dry-inflector-style: Regexp rule arrays, `gsub` with `\1` backreferences, block gsub with `$1`, camelize/underscore/dasherize/humanize); `module_lifecycle_hooks` regression covers `inherited`/`included`/`prepended`/`extended` hooks including super-forwarding and singleton method definition on individual objects; `super_class_method_args` regression covers multi-level super chains in class methods; 633 regressions green
- 2026-06-01: 700-test compatibility stop — `String#getbyte` implemented, `String#setbyte` now updates the receiver binding and returns the written byte, and `String#each_byte` uses `byte_len` so embedded NUL bytes are no longer skipped; added focused regressions for byte strings, `String#scrub`/`casecmp` edges, `Forwardable`, `OpenStruct`, runtime constants/ENV/Process, `Pathname#stat`/`chmod`, `ObjectSpace`/`GC` stubs, module constant APIs, `Thread::Mutex`/`Monitor`, `Date`, `JSON`, `Set`, `Regexp.escape`/`union`, `StringIO`, `Class#allocate`, and `byteslice` edge cases; 700 regressions green
- 2026-06-01: 750-test compatibility stop — added deterministic coverage for `Etc`, `Timeout`, `Zlib`, `StringScanner`, `Tempfile` cursor/line/suffix helpers, `MonitorMixin`, RubyGems `Version`/`Requirement`/`Dependency`/`Specification`/`Platform`/`NameTuple`/path helpers, encoding aliases, seeded `Random`, extra `Math`, `Dir.chdir` block restoration, `File.open` block returns, nested `dig`, hash/array lookup helpers, lazy/enumerator state, integer/rational/complex numeric edges, string transforms/splitting/regex globals/Unicode byte views, regexp named captures/scan/substitution, module ancestor/visibility/method parameter reflection, unbound method binding, and exception helpers; 750 regressions green
- 2026-06-01: 800-test compatibility stop — added coverage for date boundary arithmetic, JSON parse/generate edges, `Set` add/delete, `StringIO` seeking and EOF behavior, `Pathname` dirname/stat errors, append-mode files, non-block `Dir.chdir`, directory children/globs, struct keyword/deconstruct/iteration helpers, Enumerable slices/filtering/tally/grouping, Array combinatorics/mutators/sorting/sum, Hash transforms/slicing/lookup/counting, Range first/last/step/string/bsearch behavior, singleton object methods, `respond_to_missing?`-backed `method`, method-added hook ordering, Proc/method curry/composition/parameters, Fiber resume/yield/dead-fiber errors, Symbol/Nil/Time/ENV/runtime constants, and `defined?`; 800 regressions green
- 2026-06-01: 850-test compatibility stop — added coverage for keyword splat merging, yield/block forwarding, array/hash mutator and lookup edges, string slicing/insertion/justification/line/succ/predicate/numeric parsing helpers, regexp option/matchdata/gsub block-global behavior, integer/float/numeric predicate and component helpers, IO rewind/seek/byte/sync behavior, RubyGems source/config/hook/path helpers, `OpenStruct` assignment, `Forwardable` to String methods, `Timeout` custom-error passthrough, `Etc.getpwnam`, `StringScanner` match state, `Zlib::GzipReader` line delegation, `Tempfile#close!`, and class/module lifecycle hooks; 850 regressions green
- 2026-06-01: 900-test compatibility stop — added another fixture sweep across Enumerable predicates/minmax/reduce/object accumulation, Range membership and aggregation, String enumerators/bang mutators/count/delete/squeeze/case/replace/dump-undump, pack/unpack, MatchData named and positional access, Regexp construction/interpolation, alias/attr/module visibility helpers, file path/metadata/delete/expand helpers, Thread and Process status helpers, Bundler dependency/settings/lockfile/source helpers, object equality/hash/freeze/id behavior, and control-flow/assignment/lambda/symbol-to-proc forms; 900 regressions green
- 2026-06-01: 950-test compatibility stop — added coverage for Array combinatoric, fetch, bsearch, values_at, assoc/rassoc, take/drop, and to_h edges; Hash merge/default/transform/slice/except/compact/reverse_each/to_h paths; Enumerable chunking, slice_before, lazy chaining, grouping, tally, and each_with_index behavior; Range step/reverse iteration; Integer/Float/Numeric/Rational/Complex helpers; String tr/scan/partition/delete-prefix/suffix/casecmp/sub-hash behavior; Regexp/MatchData named and indexed access; singleton method, class variable, constant lookup, Struct, StringIO, Pathname, Set, Date, JSON, and OpenStruct shim coverage; 950 regressions green
- 2026-06-01: 1000-test compatibility stop — added another stable breadth pass for Array push/pop/shift/unshift/delete/insert/fill/rotate/join/compact/set operations; Hash store/delete/key/value/values_at/assoc/rassoc/clear/replace/select/reject/delete_if behavior; String reverse/chomp/chop/justify/index/byteslice/split/upto/hex/oct/match/encoding/concat paths; Regexp source/options/union and MatchData pre/post/begin/end access; Integer/Float predicates and rounding; control-flow modifiers, ternary, case-regexp matching, object ivar/send helpers, and StringIO/Set/Date/JSON/OpenStruct shims; 1000 regressions green
...
- 2026-05-19: Stage 4/6 compatibility sweep (round 6) — pattern matching scope fixed (sema collect/resolve phases now handle NODE_PATCASE; captured variables visible after case block); Struct#deconstruct_keys for hash patterns; String#byteslice; String#undump; Integer#<=> returns nil for incomparable types; String#rindex regex; 595 regressions green
- 2026-05-19: Stage 4/6 compatibility sweep (round 5) — `String#rindex` accepts Regexp; `Exception#backtrace_locations` returns `Thread::Backtrace::Location` objects; integer floor division fixed (`-7/2=-4`); `Complex#to_s` no longer wraps in parens, `inspect` retains parens; pack/unpack `G`/`g` (big-endian double/float); `Enumerable#tally(hash)` accumulator variant; `Float#to_c`; `Module#prepended` hook; 594 regressions green
- 2026-05-19: Stage 4/6 compatibility sweep (round 4) — `Array#one?`; `String#index` accepts Regexp + offset; `Enumerable#tally(hash)` optional accumulator hash (Ruby 3.1+); `Module#prepended` hook; `Float#to_c`; 594 regressions green
- 2026-05-19: Stage 4/6 compatibility sweep (round 3) — `dispatch_hash` now falls through to Enumerable (was raising NoMethodError for unknown methods, blocking all inherited methods); `Hash#each` with single-param block now yields `[key,value]` pair arrays (was yielding key only); `method()` for primitive types now populates `__method__` from inherited Ruby methods; `Module#prepended` hook fires; `Float#to_c`; `ObjectSpace.each_object` returns empty Enumerator stub; required keyword params now correctly reported as `:keyreq` in `Method#parameters` (was always `:key`); `count_required_params` now counts required keywords (keyreq) in method arity; `H/h` unpack format added; 594 regressions green
- 2026-05-19: Stage 4/6 compatibility sweep (round 2) — `loop` now calls `eval_clear_exception()` when swallowing `StopIteration` so the full exception state (including `exception_msg`) is cleared, preventing subsequent `rescue` clauses from missing exceptions after a `loop`; symbol `object_id` now stable (FNV-1a hash of name string; lexer intern() does not deduplicate); `caller_locations` now returns `Thread::Backtrace::Location` objects (with `.label`, `.lineno`, `.path`) instead of plain strings; `Thread::Backtrace::Location` class added to prelude; `IO.select` implemented via POSIX `select(2)`; `Integer ** negative_integer` now returns `Rational` per Ruby 3+ semantics; `Rational#to_s` now outputs `1/2` (no parens; was `(1/2)`); `Rational#inspect` unchanged `(1/2)`; `Random` class added to prelude (LCG PRNG, seeded instances, `Random::DEFAULT`); `Integer#ceildiv` (Ruby 3.2+); `Module#method_added`/`extended`/`included` hooks all fire; private method external-call error messages now say "private method '...' called for an instance of" matching MRI; `Hash#inspect` and `dispatch_inspect` now uses `{key: val}` format for symbol keys matching MRI; `Integer/Float` ancestor ordering fixed (`Comparable` now through `Numeric`, not direct include, giving `[Integer, Numeric, Comparable, ...]`); `Integer#bit_length`; `defined?(assignment_expr)` returns "assignment"; `Hash#fetch` KeyError message now MRI-format "key not found: :key"; adjacent string literal concatenation `"hello " "world"` works including across backslash-newline continuation; `Module#const_missing` hook fires on `Foo::Bar` constant lookup failure; `Module#const_set`; `Integer#ord`/`#infinite?`/`#finite?`/`#nan?`/`#real?`/`#integer?`/`#real`/`#imaginary` added; `Float#real?`/`#integer?`/`#real`/`#imaginary` added; `Integer#chr` now handles full Unicode; `String#tr_s`; pack/unpack N/n/V/v/f/d formats added; `String#delete_prefix!`/`delete_suffix!`; `String#each_char`/`each_byte` blockless returns Enumerator; `String#each_line` blockless returns Enumerator; `String#lines(chomp: true)` kwarg; `match?` no longer sets `$~`; `p()`/`pp()` no-args returns nil; 594 regressions green
...
- ~~**Heredocs** (`<<~HEREDOC` / `<<HEREDOC`): not implemented in lexer~~ done (`<<IDENT`, `<<~IDENT`, `<<"IDENT"`, `<<'IDENT'`, `<<~'IDENT'`), including same-line trailing tokens after the marker
...
- ~~`Float#to_r` still returns self rather than a true Rational approximation~~ done — finite floats now convert to an exact binary rational when representable within the current integer-backed Rational model, and non-finite cases raise `FloatDomainError`
