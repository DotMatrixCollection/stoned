...
- Enumerable: `find_index`/`index`, `reverse_each`, `filter_map`, `grep`, `grep_v`, `find_all`/`filter` aliases, `one?`, `each_slice(n)`, `each_cons(n)`, `take_while`, `drop_while`; `each_slice` and `each_cons` return an array when called without a block
