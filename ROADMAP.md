...
|---|---|---|
| Tiny pure-Ruby gem lifecycle | `colorize` 1.1.0 installs end-to-end; `require "colorize"` + `require "colorized_string"` work; `"hi".red/.blue.bold/.green`, `colorize(:color=>:magenta,:background=>:cyan)`, `uncolorize`, `ColorizedString.new("hi").red/.blue.bold`, `ColorizedString["world"].green`, `ColorizedString#length`/`upcase`/`to_s` all produce correct output; multi-digit octal escapes (`\033`), CVAR semantics, `String.new` dispatch, and `String` subclass built-in method proxying all fixed | Next: add more zero-dep gems to the probe fixture; `String.color_samples` (uses `permutation`+`new(str)` path) |
