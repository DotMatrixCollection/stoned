...
- Bundler bringup now covers local install/check/list/exec flows, lockfile drift failures, `gemspec` runtime-dependency path overrides, path-gem runtime dependency closure against the local gem home, grouped dependency filtering via `BUNDLE_WITH` / `BUNDLE_WITHOUT`, `bundle exec` command lookup for gem executables, reproducible lockfiles that retain excluded path-group entries, local gem build/install preserving installed gemspec runtime dependency metadata, and RubyGems reloading those installed runtime dependencies through `Gem::Specification.find_by_name`
