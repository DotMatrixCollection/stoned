...
| Method / ancestor / visibility matrix | Initial regression now covers include+prepend ancestor order, `super` chaining, protected/private `respond_to?`, `public_send`, private `respond_to_missing?`, and `Method#super_method`/`Method#call` across prepended modules | Extend the matrix with singleton class/module-function cases and Ruby 4.0.4 class-side `extend` ordering |
