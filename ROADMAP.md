...
- receiverless implicit-self DSL calls inside `instance_eval`-style blocks; today some Bundler probes still need explicit receivers (`g.source`, `g.gem`) where MRI-style `source` / `gem` should work
