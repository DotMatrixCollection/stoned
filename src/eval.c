...
void eval_init(Eval *ev, Arena *arena, FILE *out, const char *current_file, const char *exec_path,
    ...
    static const char *prelude_enumerable =
        "module Enumerable\n"
        "  def find\n"
        "    return Enumerator.new(entries) unless block_given?\n"
        "    result = nil\n"
        "    each do |x|\n"
        "      if yield x\n"
        "        result = x\n"
        ...
        "\n"
        "  def find_index(*args)\n"
        "    return to_enum(:find_index) if args.length == 0 && !block_given?\n"
        "    i = 0\n"
        "    each do |x|\n"
        "      matched = args.length > 0 ? (x == args[0]) : yield(x)\n"
        "      return i if matched\n"
        "      i = i + 1\n"
        "    end\n"
        "    nil\n"
        ...
        "    entries.reverse_each { |x| yield x }\n"
        "    self\n"
        "  end\n"
        "\n"
        "  def first(n = nil)\n"
        ...
        "\n"
        "  def cycle(n = nil)\n"
        "    return Enumerator.new([]) if !block_given? && n.nil?\n"
        ...
        "\n"
        "  def one?(*args)\n"
        "    pattern = args.length > 0 ? args[0] : nil\n"
        "    matched = 0\n"
        "    each do |x|\n"
        "      ok = args.length > 0 ? (pattern === x) : (block_given? ? yield(x) : x)\n"
        "      if ok\n"
        "        matched = matched + 1\n"
        ...
        "  def min_by(n = nil)\n"
        "    return Enumerator.new(entries) unless block_given?\n"
        "    pairs = map { |x| [x, yield(x)] }\n"
        "    pairs.sort! { |a, b| a[1] <=> b[1] }\n"
        "    n ? pairs.first(n).map { |p| p[0] } : (pairs.empty? ? nil : pairs[0][0])\n"
        "  end\n"
        "\n"
        "  def max_by(n = nil)\n"
        "    return Enumerator.new(entries) unless block_given?\n"
        ...
        "\n"
        "  def zip(*others, &blk)\n"
        "    blk ? entries.zip(*others, &blk) : entries.zip(*others)\n"
        "  end\n"
        "\n"
        "  def group_by\n"
        "    return Enumerator.new(entries) unless block_given?\n"
        ...
        "        group << x\n"
        "      else\n"
        "        result << [prev_key, group]\n"
        "        group = [x]\n"
        "      end\n"
        "      prev_key = key\n"
        "      first = false\n"
        "    end\n"
        "    result << [prev_key, group] unless first\n"
        "    result\n"
        "  end\n"
        "\n"
        "  def chunk_while\n"
        ...
        "      first = false\n"
        "    end\n"
        "    result << group unless first\n"
        "    result\n"
        "  end\n"
        "\n"
        "  def slice_when\n"
        "    return to_enum(:slice_when) unless block_given?\n"
        "    result = []\n"
        "    group = []\n"
        "    prev = nil\n"
        "    first = true\n"
        ...
        "  end\n"
        "\n"
        "  def slice_before(pattern = nil)\n"
        "    return to_enum(:slice_before) if pattern.nil? && !block_given?\n"
        "    result = []\n"
        "    group = []\n"
        "    each do |x|\n"
        "      cut = block_given? ? yield(x) : (x == pattern)\n"
        "      if cut && !group.empty?\n"
        "        result << group\n"
        "        group = []\n"
        "      end\n"
        ...
        "    return to_enum(:slice_after, pattern) unless block_given? || !pattern.nil?\n"
        "    result = []\n"
        "    group = []\n"
        "    each do |x|\n"
        "      group << x\n"
        "      matched = block_given? ? yield(x) : (x == pattern)\n"
        "      if matched\n"
        "        result << group\n"
        "        group = []\n"
        "      end\n"
    ...
    static const char *prelude_core =
        ...
        "    klass = Class.new\n"
        "    klass.instance_variable_set(:@__members, members)\n"
        "    def klass.members; @__members; end\n"
        "    def klass.new(opts = {})\n"
        "      obj = allocate\n"
        "      obj.__send__(:initialize, opts)\n"
        "      obj\n"
        "    end\n"
        "    klass.class_eval do\n"
        "      def initialize(opts = {})\n"
        "        self.class.members.each { |m| instance_variable_set(\"@#{m}\", opts[m]) }\n"
        "        freeze\n"
        "      end\n"
        "      def method_missing(name, *args)\n"
        "        return instance_variable_get(\"@#{name}\") if args.empty? && self.class.members.include?(name)\n"
        "        super\n"
        "      end\n"
        "      def respond_to_missing?(name, include_private = false)\n"
        "        self.class.members.include?(name) || super\n"
        "      end\n"
        "      def ==(other)\n"
        "        other.class == self.class && self.class.members.all? { |m| __send__(m) == other.__send__(m) }\n"
        "      end\n"
        "      def to_h\n"
        "        h = {}\n"
        "        self.class.members.each { |m| h[m] = __send__(m) }\n"
        "        h\n"
        "      end\n"
        "      def with(opts = {})\n"
        "        vals = {}\n"
        "        self.class.members.each { |m| vals[m] = opts.key?(m) ? opts[m] : __send__(m) }\n"
        "        self.class.new(vals)\n"
        "      end\n"
        "      def deconstruct; self.class.members.map { |m| __send__(m) }; end\n"
        "      def deconstruct_keys(keys)\n"
        "        h = {}\n"
        "        (keys || self.class.members).each { |m| h[m] = __send__(m) if self.class.members.include?(m) }\n"
        "        h\n"
        "      end\n"
        "    end\n"
        "    klass\n"
        ...
        "    i = offset\n"
        "    result = []\n"
        "    @arr.each { |x| result << blk.call(x, i); i += 1 }\n"
        "    result\n"
        "  end\n"
    ...
    static const char *prelude_signal =
        ...
        "end\n"
        "class ARGF_CLASS\n"
        "  def initialize; @argv = ARGV.dup; @buf = ''; end\n"
        "  def read; @buf; end\n"
        "  def readlines; @buf.lines; end\n"
        "  def each_line(&blk); @buf.each_line(&blk); end\n"
        ...
        "    0\n"
        "  end\n"
        "  def self._id2ref(id); raise RangeError, 'id not found'; end\n"
    ...
    static const char *prelude_rational =
        ...
        "  def deconstruct; [@numerator, @denominator]; end\n"
        "  def deconstruct_keys(keys)\n"
        "    h = {numerator: @numerator, denominator: @denominator}\n"
        "    keys ? h.slice(*keys) : h\n"
        "  end\n"
        "\n"
        ...
        "  end\n"
        "\n"
        "  def deconstruct; [@real, @imaginary]; end\n"
        "  def deconstruct_keys(keys)\n"
        "    h = {real: @real, imaginary: @imaginary}\n"
        "    keys ? h.slice(*keys) : h\n"
        "  end\n"
        "end\n"
        "\n"
