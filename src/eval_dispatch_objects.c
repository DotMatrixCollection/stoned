...
static const char *primitive_methods_for_class(const char *klass_name) {
    ...
    if (strcmp(klass_name, "Hash") == 0)
        return "keys,values,length,size,empty?,has_key?,has_value?,key?,value?,include?,member?,fetch,fetch_values,merge,merge!,update,delete,clear,store,key,assoc,rassoc,values_at,default,default=,default_proc,default_proc=,rehash,each,reverse_each,each_pair,each_key,each_value,map,select,find_all,filter,select!,filter!,keep_if,reject,reject!,delete_if,grep,grep_v,partition,any?,all?,none?,one?,count,sum,flat_map,collect_concat,find,detect,find_index,cycle,min_by,max_by,sort_by,group_by,tally,chunk,chunk_while,slice_when,slice_before,each_with_object,transform_keys,transform_values,transform_keys!,transform_values!,to_a,to_h,to_hash,invert,compact,compact!,slice,except,inspect,to_s,freeze,frozen?,dup,clone,replace,[],[]=";
