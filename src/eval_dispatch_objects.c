...
static const char *primitive_methods_for_class(const char *klass_name) {
    ...
    if (strcmp(klass_name, "Hash") == 0)
        return "keys,values,length,size,empty?,has_key?,has_value?,key?,value?,include?,member?,fetch,merge,merge!,update,delete,each,each_pair,each_key,each_value,map,select,find_all,filter,reject,grep,grep_v,any?,all?,none?,one?,count,sum,flat_map,find,detect,min_by,max_by,sort_by,group_by,each_with_object,transform_keys,transform_values,transform_keys!,transform_values!,to_a,to_h,invert,inspect,to_s,freeze,frozen?,dup,clone,replace,[],[]=";
    if (strcmp(klass_name, "Range") == 0)
        return "begin,first,end,last,exclude_end?,include?,member?,cover?,===,each,each_with_index,to_a,entries,size,count,length,min,max,step,map,collect,select,find_all,filter,reject,grep,grep_v,one?,to_s,inspect";
