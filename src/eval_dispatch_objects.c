...
static const char *primitive_methods_for_class(const char *klass_name) {
    ...
    if (strcmp(klass_name, "Array") == 0)
        return "length,size,count,empty?,first,last,push,pop,shift,unshift,append,prepend,<<,+,-,&,|,*,flatten,compact,compact!,uniq,sort,sort_by,reverse,reverse_each,map,collect,select,find_all,filter,reject,grep,grep_v,partition,each,each_with_index,each_with_object,each_slice,each_cons,flat_map,collect_concat,inject,reduce,zip,product,combination,permutation,repeated_combination,repeated_permutation,transpose,assoc,rassoc,sample,shuffle,cycle,include?,index,find_index,rindex,to_a,to_ary,join,min,max,min_by,max_by,minmax,minmax_by,sum,any?,all?,none?,one?,count,tally,group_by,chunk,chunk_while,slice_when,slice_before,rotate,rotate!,take,take_while,drop,drop_while,flatten,flatten!,inspect,to_s,freeze,frozen?,dup,clone,replace,pack,[],intersection,union,difference,delete_at,insert";
    if (strcmp(klass_name, "Hash") == 0)
        return "keys,values,length,size,empty?,has_key?,has_value?,key?,value?,include?,member?,fetch,fetch_values,merge,merge!,update,delete,each,reverse_each,each_pair,each_key,each_value,map,select,find_all,filter,reject,grep,grep_v,partition,any?,all?,none?,one?,count,sum,flat_map,collect_concat,find,detect,find_index,cycle,min_by,max_by,sort_by,group_by,tally,chunk,chunk_while,slice_when,slice_before,each_with_object,transform_keys,transform_values,transform_keys!,transform_values!,to_a,to_h,invert,compact,compact!,slice,except,inspect,to_s,freeze,frozen?,dup,clone,replace,[],[]=";
