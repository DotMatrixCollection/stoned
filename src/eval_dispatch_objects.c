...
static const char *primitive_methods_for_class(const char *klass_name) {
    ...
    if (strcmp(klass_name, "String") == 0)
        return "to_s,to_i,to_f,to_r,to_c,to_sym,to_str,length,size,empty?,upcase,upcase!,downcase,downcase!,capitalize,swapcase,strip,strip!,lstrip,lstrip!,rstrip,rstrip!,chomp,chomp!,chop,chop!,chars,bytes,lines,split,join,include?,start_with?,end_with?,index,rindex,[],[]=,slice,slice!,replace,reverse,reverse!,center,ljust,rjust,count,delete,delete!,squeeze,squeeze!,tr,tr!,scan,sub,sub!,gsub,gsub!,match,match?,=~,casecmp,casecmp?,partition,rpartition,ord,hex,oct,succ,next,encode,encoding,freeze,frozen?,dup,clone,inspect,hash,<<,+,*,concat,each_line,each_char,each_byte,insert,prepend,delete_prefix,delete_suffix,b,unicode_normalize,force_encoding,valid_encoding?,ascii_only?,bytesize,byteslice,unpack,unpack1,setbyte,getbyte";
    ...
    if (strcmp(klass_name, "Proc") == 0)
        return "call,[],lambda?,arity,parameters,to_proc,curry,to_s,inspect";
    ...
    if (strcmp(klass_name, "MatchData") == 0)
        return "to_s,inspect,[],captures,names,named_captures,begin,end,pre_match,post_match";
    if (strcmp(klass_name, "Exception") == 0)
        return "message,to_s,inspect,exception,backtrace,set_backtrace";
...
const char *primitive_class_methods_for_class(const char *klass_name) {
    ...
    if (strcmp(klass_name, "Time") == 0)
        return "new,now,at,local,mktime,gm,utc";
    return NULL;
}
