...
- Method reflection: `Object#method`, `Class#instance_method`, `Method#name`/`owner`/`receiver`/`arity`/`parameters`/`source_location`/`unbind`, `UnboundMethod#name`/`owner`/`arity`/`parameters`/`source_location`/`bind`/`bind_call`, plus method-object equality and hashing semantics for repeated lookups
