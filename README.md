...
- Proc/lambda: `Proc.new {}`, `proc {}`, `lambda {}`, `-> (...) {}`, `call`, `[]`, `lambda?`, `arity`, lambda `return`, proc non-local `return`; `Symbol#to_proc` and `&:symbol` block-pass in calls; `&proc` block-pass; `*arr` splat args in calls; `block_given?`; `Object#itself`
