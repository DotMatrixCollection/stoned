...
Right now it is past the "barely starts" stage and into "serious prototype" territory. The parser, semantic pass, evaluator, and regression suite are all in place, `527` tests are passing, birb's REPL fully evaluates expressions and echoes results, and a substantial RubyGems/Bundler bringup path now exists for compatibility probes. It is still honest about not being Ruby-complete yet.
...
Current coverage in the tree: `527 passed, 0 failed, 527 total`. Recent additions include `class_eval`/`module_eval` block form correctly hoisting `def` into the class, `instance_exec`, rescuable `SystemExit` (exit/abort raise instead of calling C `exit()` directly), multi-parameter custom exception `initialize` now working, `SystemExit` and `IndexError` in the builtin hierarchy, plus the bundler bringup work: the minimal RubyGems shim (`require "rubygems"` plus key sub-requires), `exe/gem` and `exe/bundle` bringup helpers, multiline `/.../x` regexp lexing, and newer Bundler/RubyGems bringup coverage around local `gem search`, filtered `gem list`, `gem contents`, `gem specification`, `gem open`, `gem which`, `gem unpack`, version-aware `Gem.bin_path` / `Gem.activate_bin_path`, version-aware RubyGems activation/lookups, version-selectable installed-gem inspection and uninstall commands, uninstall-time executable wrapper cleanup, missing-version error reporting for installed gems, missing-argument validation for uninstall, lockfile drift, installed- and path-gem `bundle exec` command resolution, missing-command validation for `bundle exec`, missing-lockfile handling for `bundle exec`, `bundle check`, `bundle list`, `bundle show`, and `bundle info`, `bundle show` / `bundle info` / `bundle open` path introspection for both path and installed gems, required-gem validation for `bundle info`, `gemspec` path overrides, path-gem runtime dependency closure, installed gemspec runtime-dependency plus executable metadata preservation, installed-spec runtime dependency loading, `BUNDLE_WITH` / `BUNDLE_WITHOUT` group handling, broader Bundler shim object coverage around `definition`, `runtime`, `settings`, sources/specs/lockfiles, config-file-backed `Bundler::Settings`, richer `bundler/inline` support for `ruby` and nested `eval_gemfile`, and common `require "bundler/..."` compatibility files. the minimal RubyGems shim (`require "rubygems"` plus key sub-requires), `exe/gem` and `exe/bundle` bringup helpers, multiline `/.../x` regexp lexing, and newer Bundler/RubyGems bringup coverage around local `gem search`, filtered `gem list`, `gem contents`, `gem specification`, `gem open`, `gem which`, `gem unpack`, version-aware `Gem.bin_path` / `Gem.activate_bin_path`, version-aware RubyGems activation/lookups, version-selectable installed-gem inspection and uninstall commands, uninstall-time executable wrapper cleanup, missing-version error reporting for installed gems, missing-argument validation for uninstall, lockfile drift, installed- and path-gem `bundle exec` command resolution, missing-command validation for `bundle exec`, missing-lockfile handling for `bundle exec`, `bundle check`, `bundle list`, `bundle show`, and `bundle info`, `bundle show` / `bundle info` / `bundle open` path introspection for both path and installed gems, required-gem validation for `bundle info`, `gemspec` path overrides, path-gem runtime dependency closure, installed gemspec runtime-dependency plus executable metadata preservation, installed-spec runtime dependency loading, `BUNDLE_WITH` / `BUNDLE_WITHOUT` group handling, broader Bundler shim object coverage around `definition`, `runtime`, `settings`, sources/specs/lockfiles, config-file-backed `Bundler::Settings`, richer `bundler/inline` support for `ruby` and nested `eval_gemfile`, and common `require "bundler/..."` compatibility files.
...
- Built-in mixins: `Comparable` (`<`, `<=`, `>`, `>=`, `between?`, `clamp`) and `Enumerable` (`find`, `detect`, `entries`, `to_a`, `first`, `take`, `drop`, `count`, `map`/`collect`, `select`, `reject`, `reduce`/`inject`, `any?`, `all?`, `none?`, `include?`/`member?`, `min`, `max`, `minmax`, `min_by`, `max_by`, `minmax_by`, `sort`, `sort_by`, `sum`, `flat_map`, `each_with_object`, `zip`, `group_by`, `tally`, `filter_map`, `each_slice`, `each_cons`, `take_while`, `drop_while`)
...
- Method reflection: `Object#method`, `Class#instance_method`, `Method#name`/`owner`/`receiver`/`arity`/`parameters`/`source_location`/`super_method`/`unbind`, `UnboundMethod#name`/`owner`/`arity`/`parameters`/`source_location`/`super_method`/`bind`/`bind_call`, plus method-object equality and hashing semantics for repeated lookups
...
- RubyGems/Bundler support is still bringup-grade: the previously blocking interpreter gaps (`class_eval`, `instance_exec`, rescuable `exit`, multi-param custom exceptions) are now fixed, but full dependency resolution, activation semantics, native extensions, and real-world gem installation still need work
