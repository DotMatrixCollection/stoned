...
Right now it is past the "barely starts" stage and into "serious prototype" territory. The parser, semantic pass, evaluator, and regression suite are all in place, `256` tests are passing, and recent work extended control-flow syntax, added missing Enumerable/collection methods, and fixed string range slicing, but it is still honest about not being Ruby-complete yet.
...
Current coverage in the tree: `256 passed, 0 failed, 256 total`.
