...
- IO: `$stdout`, `$stderr`, `$stdin`, `STDOUT`, `STDERR`, `STDIN` as IO objects with `puts`, `print`, `write`, `<<`, `flush`, `sync`, `sync=`, `fileno`, `isatty` / `tty?`, `close`, `closed?`, `tell`, `seek`, `rewind`, `$stdin.gets` / `$stdin.gets(sep)` / `$stdin.gets(nil)`, `$stdin.read`, and `IO.new(fd, mode)` wrappers with mode enforcement; `sync` / `sync=` state now persists on `IO`/`File`; kernel `puts` / `print` / `p` / `pp` now honor `$stdout`/`STDOUT` close state and write-capable `$stdout` redirection; invalid `$stdout`/`$stderr` assignment raises `TypeError`; uncaught runtime errors honor redirected `$stderr`; mode violations raise `IOError`, closed-stream access raises `IOError`
- File: `File.read`, `File.write`, `File.open` (block and non-block), `File.delete`, `File.exist?`; file objects with stateful native handles for `read`, `gets`, `write`, `print`, `puts`, `path`, `mode`, `tell`, `seek`, `rewind`, `close`, `closed?`; text modes `r`, `w`, `a` and binary modes `rb`, `wb`, `ab` — binary mode skips UTF-8 validation so non-UTF-8 byte sequences read without error; file-not-found raises `Errno::ENOENT`
...
- File/IO coverage is now stateful enough for real cursors and descriptor wrappers, but it is still well short of MRI: no socket IO and still-limited encoding/mode fidelity outside the covered text/binary and `gets` separator behavior
