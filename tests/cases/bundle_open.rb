...
Dir.chdir(app) do
  ...
  puts "--OPEN-MAIN--"
  main_out, main_status = capture("#{RbConfig.ruby.inspect} #{bundle_exe.inspect} open main")
  puts main_out
  puts main_status

  puts "--OPEN-OPT--"
  opt_out, opt_status = capture("#{RbConfig.ruby.inspect} #{bundle_exe.inspect} open opt")
  puts opt_out
  puts opt_status

  puts "--OPEN-USAGE--"
  usage_out, usage_status = capture("#{RbConfig.ruby.inspect} #{bundle_exe.inspect} open")
  puts usage_out
  puts usage_status
end
