...
root = "/tmp/stoned_gem_cli_specification_#{$$}"
...
File.write(File.join(project, "demo.gemspec"), <<~GEMSPEC)
  Gem::Specification.new do |s|
    ...
    s.files = ["lib/demo.rb"]
    s.require_paths = ["lib"]
    s.add_runtime_dependency "dep", ">= 0.5"
  end
GEMSPEC
...
Dir.chdir(project) do
  puts `#{RbConfig.ruby} #{gem_exe} build demo.gemspec`.lines.first.chomp
  puts `#{RbConfig.ruby} #{gem_exe} install demo-1.2.3.gem`.chomp
  puts "--SPEC--"
  puts `#{RbConfig.ruby} #{gem_exe} specification demo`.chomp
end
